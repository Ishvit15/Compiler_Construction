...
// To output the parse tree to a text file
void printParseTreeNode(treeNode *rootTreeNode)
{
    
    // ptOutFile = fopen("parsetreeOutFile.txt", "a+");
    //fseek(ptOutFile, 0, SEEK_SET);
    if (ptOutFile == NULL)
    {
        printf("Error while creating the parse tree output file.\n");
        return;
    }
    if (rootTreeNode->symbol.isTerminal)
    {
        if (rootTreeNode->token.TK == NUM)
            fprintf(ptOutFile, "%d ", rootTreeNode->token.value_if_int);
        else if (rootTreeNode->token.TK == RNUM)
            fprintf(ptOutFile, "%d ", rootTreeNode->token.value_if_float);
        else
            fprintf(ptOutFile, "%s ", rootTreeNode->token.lexeme);
    }
    else
        fprintf(ptOutFile, "NA ");

    if (rootTreeNode->symbol.isTerminal)
        fprintf(ptOutFile, "(Line no. - %d) ", rootTreeNode->token.line_no);
    else
        fprintf(ptOutFile, "NA ");

    if (rootTreeNode->symbol.isTerminal)
        fprintf(ptOutFile, "%s ", enumToTerminal[rootTreeNode->token.TK]);
    else
        fprintf(ptOutFile, "%s ", enumToNonTerminal[rootTreeNode->symbol.ntno]);
    if (rootTreeNode->childCount == 0)
        fprintf(ptOutFile, "OVER ");
    fprintf(ptOutFile, "\n");
    //fclose(ptOutFile);
}
...
void initParser(FILE *fp, int len, char* fn)
{
    ...
    treeNode *root = parseUserSourceCode(fcp);
    ptOutFile = fopen("parsetreeOutFile.txt", "a+");
    printParseTree(root);
    fclose(fcp);
    fclose(ptOutFile);
